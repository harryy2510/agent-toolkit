...
Every source-like file discovered by the repo intelligence scanner. Tags are heuristic and meant to guide read paths.
...
- `bin/agent-toolkit.ts` — 6 lines — tags: ast
...
- `scripts/bump-version.ts` — 76 lines — tags: ast

## `src/cli.ts`

- `src/cli.ts` — 9 lines — tags: ast
...
- `src/platform.ts` — 10 lines — tags: ast

## `test/bump-version.test.ts`

- `test/bump-version.test.ts` — 57 lines — tags: test, ast
