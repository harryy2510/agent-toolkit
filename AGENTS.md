...
- Commit `.agents/intel/` when produced for migrated repos so repo intelligence can travel with the project.
- Do not commit `.agents/local.json` or `.agents/generated/`; they are local/private generated state.
...
- Use the repo's active committed hook manager for git hooks: respect an existing `.husky` or `.vite-hooks` directory, default new Vite+ projects to `.vite-hooks`, and default other repos to Husky `.husky`. Do not introduce `.githooks` or ad hoc hook folders.
